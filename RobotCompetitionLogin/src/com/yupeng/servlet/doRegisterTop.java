...
public class doRegisterTop extends HttpServlet {

	/**
	 * Constructor of the object.
	 */
	public doRegisterTop() {
		super();
	}
	...
	public void doPost(HttpServletRequest request, HttpServletResponse response)
		...
		if(request.getParameter("subjectId")!=null&&!request.getParameter("subjectId").isEmpty()){
			...
			List<SubjectInfo> list=(ArrayList<SubjectInfo>)session.getAttribute("listSubjectInfo");
			if(list!=null){
				for(SubjectInfo subjectInfo:list){
					if(subjectId==subjectInfo.getSubjectId()){
						
						session.setAttribute("subjectId", subjectId);
						session.setAttribute("subjectName", subjectInfo.getSubjectName());
						session.setAttribute("peopleNumber",subjectInfo.getPeopleNumber());
						break;
					}				
				}
			}
